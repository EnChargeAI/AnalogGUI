...
class FunctionsTab(QtWidgets.QWidget):
    ...
    def _build_buck_rows(self, container: QtWidgets.QVBoxLayout):
        ...
        lr_low_voltage = self._text("0.000", "Measured low voltage")
        lr_low_voltage.setReadOnly(True)
        row.addWidget(lr_low_voltage)
        row.addWidget(QtWidgets.QLabel("High Voltage:"))
        lr_high_voltage = self._text("0.000", "Measured high voltage")
        lr_high_voltage.setReadOnly(True)
        row.addWidget(lr_high_voltage)
        row.addStretch(1)
        lr_btn = QtWidgets.QPushButton("Run")
        lr_btn.setProperty("kind", "primary")
        lr_btn.clicked.connect(lambda: self._emit("buck_line_reg", {"parameter": int(lr_param.value()), "low_voltage": lr_low_voltage.text(), "high_voltage": lr_high_voltage.text()}))
        row.addWidget(lr_btn)
        container.addLayout(row)
        ...
        row.addWidget(QtWidgets.QLabel("Low Voltage:"))
        ldr_low_voltage = self._text("0.000", "Measured low voltage")
        ldr_low_voltage.setReadOnly(True)
        row.addWidget(ldr_low_voltage)
        row.addWidget(QtWidgets.QLabel("High Voltage:"))
        ldr_high_voltage = self._text("0.000", "Measured high voltage")
        ldr_high_voltage.setReadOnly(True)
        row.addWidget(ldr_high_voltage)
        row.addStretch(1)
        ldr_btn = QtWidgets.QPushButton("Run")
        ldr_btn.setProperty("kind", "primary")
        ldr_btn.clicked.connect(lambda: self._emit("buck_load_reg", {"parameter": int(ldr_param.value()), "low_voltage": ldr_low_voltage.text(), "high_voltage": ldr_high_voltage.text()}))
        row.addWidget(ldr_btn)
        container.addLayout(row)
        ...
        row.addWidget(QtWidgets.QLabel("Low Voltage:"))
        ls_low_voltage = self._text("0.000", "Measured low voltage")
        ls_low_voltage.setReadOnly(True)
        row.addWidget(ls_low_voltage)
        row.addWidget(QtWidgets.QLabel("High Voltage:"))
        ls_high_voltage = self._text("0.000", "Measured high voltage")
        ls_high_voltage.setReadOnly(True)
        row.addWidget(ls_high_voltage)
        row.addStretch(1)
        ls_btn = QtWidgets.QPushButton("Run")
        ls_btn.setProperty("kind", "primary")
        ls_btn.clicked.connect(lambda: self._emit("buck_load_step", {"parameter": int(ls_param.value()), "low_voltage": ls_low_voltage.text(), "high_voltage": ls_high_voltage.text()}))
        row.addWidget(ls_btn)
        container.addLayout(row)
        ...
        row.addWidget(QtWidgets.QLabel("Low Voltage:"))
        vs_low_voltage = self._text("0.000", "Measured low voltage")
        vs_low_voltage.setReadOnly(True)
        row.addWidget(vs_low_voltage)
        row.addWidget(QtWidgets.QLabel("High Voltage:"))
        vs_high_voltage = self._text("0.000", "Measured high voltage")
        vs_high_voltage.setReadOnly(True)
        row.addWidget(vs_high_voltage)
        row.addStretch(1)
        vs_btn = QtWidgets.QPushButton("Run")
        vs_btn.setProperty("kind", "primary")
        vs_btn.clicked.connect(lambda: self._emit("buck_voltage_sweep", {"parameter": int(vs_param.value()), "low_voltage": vs_low_voltage.text(), "high_voltage": vs_high_voltage.text()}))
        row.addWidget(vs_btn)
        container.addLayout(row)
